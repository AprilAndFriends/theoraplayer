...
static void (*conversion_functions[])(th_img_plane*,unsigned char*,int)={0,
	decodeRGB,
	decodeRGBA,
	decodeRGBX,
	decodeARGB,
...
TheoraVideoFrame_Theora::TheoraVideoFrame_Theora(TheoraVideoClip* clip) : TheoraVideoFrame(clip)
{

}

void TheoraVideoFrame_Theora::decode(void* src, TheoraOutputMode srcFormat)
{
	conversion_functions[mParent->getOutputMode()]((th_img_plane*) src, mBuffer, mParent->getStride());
	mReady = true;
}
...
TheoraFrameQueue_Theora::TheoraFrameQueue_Theora(TheoraVideoClip* parent) : TheoraFrameQueue(parent)
{

}

TheoraVideoFrame* TheoraFrameQueue_Theora::createFrameInstance(TheoraVideoClip* clip)
{
	return new TheoraVideoFrame_Theora(clip);
}
